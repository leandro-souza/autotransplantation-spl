...
static int undo_read (bufinfo_T *bi, char_u *buffer, size_t size) {
    ...
    else if (fread (buffer, (size_t) size, 1, bi->bi_fp) != 1)
        retval = FAIL;
    if (retval == FAIL)
...
static void u_undo_end (int did_undo, int absolute) {
    ...
    smsg ((char_u *) _ ("%ld %s; %s #%ld  %s"), u_oldcount < 0 ? - u_oldcount : u_oldcount, _ (msgstr), did_undo ? _ ("before") : _ ("after"), uhp == NULL ? 0L : uhp -> uh_seq, msgbuf);
}

static void u_add_time (char_u *buf, size_t buflen, time_t tt) {
    vim_snprintf ((char *) buf, buflen, _ ("%ld seconds ago"), (long) (vim_time () - tt));
}
