...
static int throw_exception (void *value, except_type_T type, char_u *cmdname) {
    ...
    int should_free;
    if (type == ET_USER) {
        if (STRNCMP ((char_u *) value, "Vim", 3) == 0 && (((char_u *) value)[3] == NUL || ((char_u *) value)[3] == ':' || ((char_u *) value)[3] == '(')) {
...
char_u *get_exception_string (void *value, except_type_T type, char_u *cmdname, int *should_free) {
    ...
    if (type == ET_ERROR) {
        *should_free = TRUE;
        ...
        for (p = mesg;; p++) {
            if (*p == NUL || (*p == 'E' && VIM_ISDIGIT (p[1]) && (p[2] == ':' || (VIM_ISDIGIT (p[2]) && (p[3] == ':' || (VIM_ISDIGIT (p[3]) && p[4] == ':')))))) {
                if (*p == NUL || p == mesg)
                    STRCAT (val, mesg);
                else {
                ...
                if (elem->msg == NULL) {
                    ...
                    *plist = elem;
                    if (plist == msg_list || severe) {
                        char_u *tmsg;
                        tmsg = elem->msg;
                        if (STRNCMP (tmsg, "Vim E", 5) == 0 && VIM_ISDIGIT (tmsg[5]) && VIM_ISDIGIT (tmsg[6]) && VIM_ISDIGIT (tmsg[7]) && tmsg[8] == ':' && tmsg[9] == ' ')
...
static void discard_exception (except_T *excp, int was_finished) {
    ...
    if (p_verbose >= 13 || debug_break_level > 0) {
        ...
        if (debug_break_level > 0 || *p_vfile == NUL)
            msg_scroll = TRUE;
        smsg (was_finished ? (char_u *) _ ("Exception finished: %s") : (char_u *) _ ("Exception discarded: %s"), excp -> value);
        msg_puts ((char_u *) "\n");
        if (debug_break_level > 0 || *p_vfile == NUL)
            cmdline_row = msg_row;
        --no_wait_return;
...
void enter_cleanup (cleanup_T *csp) {
    ...
    if (did_emsg || got_int || did_throw || need_rethrow) {
        csp->pending = (did_emsg ? CSTP_ERROR : 0) | (got_int ? CSTP_INTERRUPT : 0) | (did_throw ? CSTP_THROW : 0) | (need_rethrow ? CSTP_THROW : 0);
    ...
    if (caught_stack != NULL) {
        ...
        if (*caught_stack->throw_name != NUL) {
            if (caught_stack->throw_lnum != 0)
                vim_snprintf ((char *) IObuff, IOSIZE, _ ("%s, line %ld"), caught_stack->throw_name, (long ) caught_stack->throw_lnum);
            else
