    ...
    while (*p != NUL) {
        if (*p != '%') {
            ...
            case 'S' :
                ...
                case 's' :
                case 'S' :
                    str_arg = tvs != NULL ? tv_str (tvs, &arg_idx, &tofree) : va_arg (ap, char *);
                    if (str_arg == NULL) {
                        str_arg = "[NULL]";
                        str_arg_l = 6;
                    }
            ...
            case 'p' :
                {
                    ...
                    if (fmt_spec == 'p') {
                        length_modifier = '\0';
                        ptr_arg = tvs != NULL ? (void *) tv_str (tvs, &arg_idx, NULL) : va_arg (ap, void *);
                        if (ptr_arg != NULL)
                            arg_sign = 1;
                    }
                    else if (fmt_spec == 'b' || fmt_spec == 'B') {
                        bin_arg = tvs != NULL ? (uvarnumber_T) tv_nr (tvs, &arg_idx) : va_arg (ap, uvarnumber_T);
                        if (bin_arg != 0)
                            arg_sign = 1;
                    }
                    else if (fmt_spec == 'd') {
                        ...
                        case 'l' :
                            ...
                            break;
                        case 'L' :
                            llong_arg = tvs != NULL ? tv_nr (tvs, &arg_idx) : va_arg (ap, varnumber_T);
                            if (llong_arg > 0)
                                arg_sign = 1;
                            else if (llong_arg < 0)
                                arg_sign = -1;
                        ...
                        case '\0' :
                        case 'h' :
                            uint_arg = tvs != NULL ? (unsigned ) tv_nr (tvs, &arg_idx) : va_arg (ap, unsigned  int);
                            if (uint_arg != 0)
                                arg_sign = 1;
                            break;
                        case 'l' :
                            ulong_arg = tvs != NULL ? (unsigned  long ) tv_nr (tvs, &arg_idx) : va_arg (ap, unsigned  long  int);
                            if (ulong_arg != 0)
                                arg_sign = 1;
                            break;
                        case 'L' :
                            ullong_arg = tvs != NULL ? (uvarnumber_T) tv_nr (tvs, &arg_idx) : va_arg (ap, uvarnumber_T);
                            if (ullong_arg != 0)
                                arg_sign = 1;
                            break;
                        }
