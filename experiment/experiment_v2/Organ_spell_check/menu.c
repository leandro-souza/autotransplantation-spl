...
static void gui_update_menus_recurse (vimmenu_T *menu, int mode) {
    ...
    while (menu) {
        if ((menu->modes & menu->enabled & mode))
            ...
            grey = TRUE;
        if (vim_strchr (p_go, GO_GREY) != NULL || menu->parent == NULL || menu_is_toolbar (menu->parent->name))
...
static int menu_name_equal (char_u *name, vimmenu_T *menu) {
    if (menu->en_name != NULL && (menu_namecmp (name, menu->en_name) || menu_namecmp (name, menu->en_dname)))
        return TRUE;
...
static int menu_namecmp (char_u *name, char_u *mname) {
    ...
    for (i = 0; name[i] != NUL && name[i] != TAB; ++i)
        if (name[i] != mname[i])
            break;
    return ((name[i] == NUL || name[i] == TAB) && (mname[i] == NUL || mname[i] == TAB));
}
...
char_u *get_menu_name (expand_T *xp, int idx) {
    ...
    while (menu != NULL && (menu_is_hidden (menu->dname) || menu_is_separator (menu->dname) || menu_is_tearoff (menu->dname) || menu->children == NULL)) {
        menu = menu->next;
...
char_u *get_menu_names (expand_T *xp, int idx) {
    ...
    while (menu != NULL && (menu_is_hidden (menu->dname) || (expand_emenu && menu_is_separator (menu->dname)) || menu_is_tearoff (menu->dname) || menu->dname[STRLEN (menu->dname) - 1] == '.')) {
        menu = menu->next;
        if (menu == NULL && !did_alt_menu) {
            menu = expand_menu_alt;
            did_alt_menu = TRUE;
...
char_u *set_context_in_menu_cmd (expand_T *xp, char_u *cmd, char_u *arg, int forceit) {
    ...
    for (p = arg; *p; ++p)
        if (!VIM_ISDIGIT (*p) && *p != '.')
            break;
    if (!VIM_ISWHITE (*p)) {
        if (STRNCMP (arg, "enable", 6) == 0 && (arg[6] == NUL || VIM_ISWHITE (arg[6])))
            p = arg + 6;
        else if (STRNCMP (arg, "disable", 7) == 0 && (arg[7] == NUL || VIM_ISWHITE (arg[7])))
            p = arg + 7;
        else
            p = arg;
    }
    ...
    if (*p == NUL) {
        ...
        while (name != NULL && *name) {
            ...
            while (menu != NULL) {
                if (menu_name_equal (name, menu)) {
                    if ((*p != NUL && menu->children == NULL) || ((menu->modes & expand_modes) == 0x0)) {
                        vim_free (path_name);
                        return NULL;
                    }
                    break;
...
static int get_menu_cmd_modes (char_u *cmd, int forceit, int *noremap, int *unmenu) {
    ...
    case 'c' :
        ...
        break;
    case 'a' :
        modes = MENU_INSERT_MODE | MENU_CMDLINE_MODE | MENU_NORMAL_MODE | MENU_VISUAL_MODE | MENU_SELECT_MODE | MENU_OP_PENDING_MODE;
        break;
    case 'n' :
        if (*cmd != 'o') {
    ...
    default :
        ...
        if (forceit)
            ...
            modes = MENU_NORMAL_MODE | MENU_VISUAL_MODE | MENU_SELECT_MODE | MENU_OP_PENDING_MODE;
    }
    if (noremap != NULL)
...
int menu_is_menubar (char_u *name) {
    return (!menu_is_popup (name) && !menu_is_toolbar (name) && !menu_is_winbar (name) && *name != MNU_HIDDEN_CHAR);
}
...
int check_menu_pointer (vimmenu_T *root, vimmenu_T *menu_to_check) {
    ...
    for (p = root; p != NULL; p = p->next)
        if (p == menu_to_check || (p->children != NULL && check_menu_pointer (p->children, menu_to_check) == OK))
            return OK;
    return FAIL;
}
...
static void execute_menu (exarg_T *eap, vimmenu_T *menu) {
    ...
    if (restart_edit && !current_SID) {
        ...
        mode = (char_u *) "Visual";
        idx = MENU_INDEX_VISUAL;
        if ((curbuf->b_visual.vi_start.lnum == eap->line1) && (curbuf->b_visual.vi_end.lnum) == eap->line2) {
    ...
    if (idx != MENU_INDEX_INVALID && menu->strings[idx] != NULL && menu->strings[idx][0] != NUL) {
        if (eap == NULL || current_SID != 0) {
