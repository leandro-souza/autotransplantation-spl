...
static void f_has (typval_T *argvars, typval_T *rettv) {
    ...
    static char * (has_list []) = {"mac", "osx", 
        ...
        "fname_case", 
        #endif
        "arabic", "autocmd", "balloon_eval", "balloon_multiline", "balloon_eval_term", "builtin_terms", 
	...
	|| defined(FEAT_GUI_MOTIF))
        "browsefilter", 
        #endif
        "byte_offset", "channel", "cindent", "clientserver", 
        ...
        "cmdline_hist", 
        #endif
        "comments", "conceal", "cryptv", "crypt-blowfish", "crypt-blowfish2", "cursorbind", 
        #ifdef CURSOR_SHAPE
        "cursorshape", 
        #endif
        ...
        "debug", 
        #endif
        "dialog_con", "dialog_gui", "diff", "digraphs", 
        #ifdef FEAT_DND
        "dnd", 
        #endif
        ...
        "libcall", 
        #endif
        "linebreak", "lispindent", "listcmds", "localmap", "menu", "mksession", "modify_fname", "mouse", "mouseshape", "mouse_dec", "mouse_netterm", "mouse_sgr", "mouse_urxvt", "mouse_xterm", 
        #ifdef FEAT_MBYTE
        "multi_byte", 
        #endif
        "multi_byte_ime", "multi_lang", "num64", 
        #ifdef FEAT_OLE
        "ole", 
        #endif
        "packages", "path_extra", "persistent_undo", "postscript", "printer", "profile", "reltime", "quickfix", "rightleft", 
        ...
        "odbeditor", 
        #endif
        "syntax", "tag_binary", "tag_old_static", "termguicolors", "terminal", "terminfo", "termresponse", "textobjects", 
        #ifdef HAVE_TGETENT
        "tgetent", 
        #endif
        "timers", "title", "toolbar", "transparency", 
        #if defined(FEAT_CLIPBOARD) && defined(FEAT_X11)
        "unnamedplus", 
        #endif
        "user-commands", "user_commands", "viminfo", "vertsplit", "virtualedit", "visual", "visualextra", "vreplace", "wildignore", "wildmenu", "windows", "writebackup", 
        #ifdef FEAT_XIM
        "xim", 
        #endif
...
static void f_inputdialog (typval_T *argvars, typval_T *rettv) {
    ...
    if (gui.in_use && vim_strchr (p_go, GO_CONDIALOG) == NULL) {
        ...
        message = get_tv_string_chk (&argvars[0]);
        if (argvars[1].v_type != VAR_UNKNOWN && (defstr = get_tv_string_buf_chk (&argvars[1], buf)) != NULL)
            vim_strncpy (IObuff, defstr, IOSIZE -1);
        else
            IObuff[0] = NUL;
        if (message != NULL && defstr != NULL && do_dialog (VIM_QUESTION, NULL, message, (char_u *) _ ("&OK\n&Cancel"), 1, IObuff, FALSE) == 1)
            rettv->vval.v_string = vim_strsave (IObuff);
        else {
            if (message != NULL && defstr != NULL && argvars[1].v_type != VAR_UNKNOWN && argvars[2].v_type != VAR_UNKNOWN)
...
static void libcall_common (typval_T *argvars, typval_T *rettv, int type) {
    ...
    if (argvars[0].v_type == VAR_STRING && argvars[1].v_type == VAR_STRING) {
        ...
        if (mch_libcall (argvars[0].vval.v_string, argvars[1].vval.v_string, string_in, argvars[2].vval.v_number, string_result, &nr_result) == OK && type == VAR_NUMBER)
...
static void f_strwidth (typval_T *argvars, typval_T *rettv) {
    char_u *s = get_tv_string (&argvars[0]);
    rettv -> vval.v_number = (varnumber_T) (#ifdef FEAT_MBYTE mb_string2cells (s, - 1) #else STRLEN (s) #endif);
}
