...
char_u *mac_string_convert (char_u *ptr, int len, int *lenp, int fail_on_error, int from_enc, int to_enc, int *unconvlenp) {
    ...
    switch (from_enc) {
    case 'l' :
        from = kCFStringEncodingISOLatin1;
        break;
    case 'm' :
        from = kCFStringEncodingMacRoman;
        break;
    case 'u' :
        from = kCFStringEncodingUTF8;
        break;
    default :
        return NULL;
    }
    switch (to_enc) {
    case 'l' :
        to = kCFStringEncodingISOLatin1;
        break;
    case 'm' :
        to = kCFStringEncodingMacRoman;
        break;
    case 'u' :
        to = kCFStringEncodingUTF8;
        break;
    default :
        return NULL;
    }
...
unsigned  short  *mac_enc_to_utf16 (char_u *from, size_t fromLen, size_t *actualLen) {
    ...
    do {
        ...
        if ((enc_canon_props (p_enc) & ENC_UNICODE) == 0 && convert_setup (&conv, p_enc ? p_enc : (char_u *) "macroman", (char_u *) "utf-8") == FAIL)
            break;
...
int enc2macroman (char_u *from, size_t fromlen, char_u *to, int *tolenp, int maxtolen, char_u *rest, int *restlenp) {
    ...
    CFIndex l;
    *restlenp = 0;
    cfstr = CFStringCreateWithBytes (NULL, from, fromlen, (enc_utf8) ? kCFStringEncodingUTF8 : kCFStringEncodingISOLatin1, 0);
    ...
    if (r.length != CFStringGetBytes (cfstr, r, kCFStringEncodingMacRoman, 0, 0, to, maxtolen, &l)) {
        CFRelease (cfstr);
        return FAIL;
    }
    CFRelease (cfstr);
...
static char_u *mac_utf16_to_utf8 (UniChar *from, size_t fromLen, size_t *actualLen) {
    ...
    if (gUTF16ToUTF8Converter) {
        ...
        if (result && TECConvertText (gUTF16ToUTF8Converter, (ConstTextPtr) from, fromLen, &inputRead, result, (fromLen * 6 + 1) * sizeof (char_u), &utf8_len) == noErr) {
...
int macroman2enc (char_u *ptr, long  *sizep, long  real_size) {
    ...
    r.location = 0;
    r.length = CFStringGetLength (cfstr);
    if (r.length != CFStringGetBytes (cfstr, r, (enc_utf8) ? kCFStringEncodingUTF8 : kCFStringEncodingISOLatin1, 0, 0, ptr +*sizep, real_size -*sizep, &len)) {
        CFRelease (cfstr);
        return FAIL;
    }
    CFRelease (cfstr);
