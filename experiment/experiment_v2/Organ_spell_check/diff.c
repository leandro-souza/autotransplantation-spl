...
static int diff_equal_char (char_u *p1, char_u *p2, int *len) {
    {
        if ((*p1 != *p2) && (!(diff_flags & DIFF_ICASE) || TOLOWER_LOC (*p1) != TOLOWER_LOC (*p2)))
            return FALSE;
        *len = 1;
    }
    return TRUE;
...
static linenr_T diff_get_corresponding_line_int (buf_T *buf1, linenr_T lnum1) {
    ...
    for (dp = curtab->tp_first_diff; dp != NULL; dp = dp->df_next) {
        ...
        if ((dp->df_lnum[idx1] == lnum1) && (dp->df_count[idx1] == 0) && (dp->df_lnum[idx2] <= curwin->w_cursor.lnum) && ((dp->df_lnum[idx2] + dp->df_count[idx2]) > curwin->w_cursor.lnum))
            return curwin->w_cursor.lnum;
        baseline = (dp->df_lnum[idx1] + dp->df_count[idx1]) - (dp->df_lnum[idx2] + dp->df_count[idx2]);
    }
    return lnum1 - baseline;
}
...
int diff_move_to (int dir, long  count) {
    ...
    while (--count >= 0) {
        ...
        for (dp = curtab->tp_first_diff;; dp = dp->df_next) {
            if (dp == NULL)
                break;
            if ((dir == FORWARD && lnum < dp->df_lnum[idx]) || (dir == BACKWARD && (dp->df_next == NULL || lnum <= dp->df_next->df_lnum[idx]))) {
                lnum = dp->df_lnum[idx];
                break;
            }
        }
