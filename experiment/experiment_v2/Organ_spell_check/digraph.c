...
char_u *keymap_init (void) {
    ...
    if (*curbuf->b_p_keymap == NUL) {
        ...
        char_u *buf;
        size_t buflen;
        buflen = STRLEN (curbuf->b_p_keymap) + 14;
...
int getdigraph (int char1, int char2, int meta_char) {
    int retval;
    if (((retval = getexactdigraph (char1, char2, meta_char)) == char2) && (char1 != char2) && ((retval = getexactdigraph (char2, char1, meta_char)) == char1))
        return char2;
    return retval;
}
