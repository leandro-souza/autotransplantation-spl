...
void prepare_viminfo_registers (void) {
    y_read_regs = (yankreg_T *) alloc_clear (NUM_REGISTERS *(int) sizeof (yankreg_T));
}
...
static void yank_do_autocmd (oparg_T *oap, yankreg_T *reg) {
    ...
    case MCHAR :
        ...
        break;
    case MBLOCK :
        vim_snprintf ((char *) buf, sizeof (buf), "%c%ld", Ctrl_V, reglen +1);
        break;
    }
    ...
    if (!VIsual_active) {
        ...
        for (; pos.lnum <= oap->end.lnum; ++pos.lnum) {
            ...
            if (netbeans_active () && one_change) {
                ...
                netbeans_inserted (curbuf, pos.lnum, pos.col, & ptr [pos.col], length);
            }
            if (g_cmd && one_change)
                amount += Prenum1;
        }
...
static int do_addsub (int op_type, pos_T *pos, int length, linenr_T Prenum1) {
    ...
    if (!VIsual_active) {
        ...
        if (dobin && dohex && !((col > 0 && (ptr[col] == 'X' || ptr[col] == 'x') && ptr[col - 1] == '0' && vim_isxdigit (ptr[col + 1])))) {
            col = pos->col;
            while (col > 0 && vim_isdigit (ptr[col])) {
                col--;
            }
        }
        if ((dohex && col > 0 && (ptr[col] == 'X' || ptr[col] == 'x') && ptr[col - 1] == '0' && vim_isxdigit (ptr[col + 1])) || (dobin && col > 0 && (ptr[col] == 'B' || ptr[col] == 'b') && ptr[col - 1] == '0' && vim_isbdigit (ptr[col + 1]))) {
            --col;
        }
        else {
            col = pos->col;
            while (ptr[col] != NUL && !vim_isdigit (ptr[col]) && !(doalp && ASCII_ISALPHA (ptr[col])))
                col += MB_PTR2LEN (ptr +col);
            while (col > 0 && vim_isdigit (ptr[col - 1]) && !(doalp && ASCII_ISALPHA (ptr[col]))) {
                --col;
            }
        }
    }
    if (visual) {
        while (ptr[col] != NUL && length > 0 && !vim_isdigit (ptr[col]) && !(doalp && ASCII_ISALPHA (ptr[col]))) {
    ...
    if (doalp && ASCII_ISALPHA (firstdigit)) {
        ...
        if (visual && VIsual_mode != 'V')
            maxlen = (curbuf->b_visual.vi_curswant == MAXCOL ? (int) STRLEN (ptr) - col : length);
        vim_str2nr (ptr + col, & pre, & length, 0 + (dobin ? STR2NR_BIN : 0) + (dooct ? STR2NR_OCT : 0) + (dohex ? STR2NR_HEX : 0), NULL, & n, maxlen);
        if (pre && negative) {
            ++col;
            --length;
            negative = FALSE;
...
void write_reg_contents_lst (int name, char_u **strings, int maxlen, int must_append, int yank_type, long  block_len) {
    ...
    if (name == '/' || name == '=') {
        ...
        if (strings[0] == NULL)
            ...
            EMSG (_ ("E883: search pattern and expression register may not " "contain two or more lines"));
            return;
        }
        else
            s = strings[0];
...
void cursor_pos_info (dict_T *dict) {
        ...
        for (lnum = 1; lnum <= curbuf->b_ml.ml_line_count; ++lnum) {
            ...
            if (VIsual_active && lnum >= min_pos.lnum && lnum <= max_pos.lnum) {
                ...
                if (s != NULL) {
                    ...
                    if (lnum == curbuf->b_ml.ml_line_count && !curbuf->b_p_eol && (curbuf->b_p_bin || !curbuf->b_p_fixeol) && (long ) STRLEN (s) < len)
        ...
        if (dict == NULL) {
            if (VIsual_active) {
                ...
                if (char_count_cursor == byte_count_cursor && char_count == byte_count)
                    vim_snprintf ((char *) IObuff, IOSIZE, _ ("Selected %s%ld of %ld Lines; %lld of %lld Words; %lld of %lld Bytes"), buf1, line_count_selected, (long ) curbuf->b_ml.ml_line_count, word_count_cursor, word_count, byte_count_cursor, byte_count);
                else
                    vim_snprintf ((char *) IObuff, IOSIZE, _ ("Selected %s%ld of %ld Lines; %lld of %lld Words; %lld of %lld Chars; %lld of %lld Bytes"), buf1, line_count_selected, (long ) curbuf->b_ml.ml_line_count, word_count_cursor, word_count, char_count_cursor, char_count, byte_count_cursor, byte_count);
            }
            else {
                p = ml_get_curline ();
                validate_virtcol ();
                ...
                if (char_count_cursor == byte_count_cursor && char_count == byte_count)
                    vim_snprintf ((char *) IObuff, IOSIZE, _ ("Col %s of %s; Line %ld of %ld; Word %lld of %lld; Byte %lld of %lld"), (char *) buf1, (char *) buf2, (long ) curwin->w_cursor.lnum, (long ) curbuf->b_ml.ml_line_count, word_count_cursor, word_count, byte_count_cursor, byte_count);
                else
                    vim_snprintf ((char *) IObuff, IOSIZE, _ ("Col %s of %s; Line %ld of %ld; Word %lld of %lld; Char %lld of %lld; Byte %lld of %lld"), (char *) buf1, (char *) buf2, (long ) curwin->w_cursor.lnum, (long ) curbuf->b_ml.ml_line_count, word_count_cursor, word_count, char_count_cursor, char_count, byte_count_cursor, byte_count);
            }
        }
        if (dict == NULL) {
            p = p_shm;
    ...
    if (dict != NULL) {
        ...
        dict_add_nr_str (dict, "bytes", byte_count, NULL);
        dict_add_nr_str (dict, VIsual_active ? "visual_bytes" : "cursor_bytes", byte_count_cursor, NULL);
        dict_add_nr_str (dict, VIsual_active ? "visual_chars" : "cursor_chars", char_count_cursor, NULL);
        dict_add_nr_str (dict, VIsual_active ? "visual_words" : "cursor_words", word_count_cursor, NULL);
    }
}
