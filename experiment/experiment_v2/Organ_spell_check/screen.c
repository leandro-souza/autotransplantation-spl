...
static int win_line (win_T *wp, linenr_T lnum, int startrow, int endrow, int nochange) {
    ...
    if (wp->w_p_spell && *wp->w_s->b_p_spl != NUL && wp->w_s->b_langp.ga_len > 0 && *(char**) (wp->w_s->b_langp.ga_data) != NULL) {
        has_spell = TRUE;
        extra_check = TRUE;
            ...
            if (extra_check) {
                ...
                if (has_spell && v >= word_end && v > cur_checked_col) {
                    ...
                    if (c != 0 && (!has_syntax || can_spell)) {
                        ...
                        len = spell_check (wp, p, &spell_hlf, &cap_col, nochange);
                        word_end = v + len;
                        if (spell_hlf != HLF_COUNT && (State & INSERT) != 0 && wp->w_cursor.lnum == lnum && wp->w_cursor.col >= (colnr_T) (prev_ptr - line) && wp->w_cursor.col < (colnr_T) word_end) {
                            spell_hlf = HLF_COUNT;
...
void screen_line (int row, int coloff, int endcol, int clear_width, int rlflag) {
    ...
    if (rlflag) {
        if (clear_width > 0) {
            while (col <= endcol && ScreenLines[off_to] == ' ' && ScreenAttrs[off_to] == 0) {
                ++off_to;
                ++col;
            }
            if (col <= endcol)
    ...
    while (col < endcol) {
        ...
        if (redraw_this) {
            if (p_wiv && !force && !gui.in_use && ScreenAttrs[off_to] != 0 && ScreenAttrs[off_from] != ScreenAttrs[off_to]) {
        ...
        else if (p_wiv && !gui.in_use && col + coloff > 0) {
    ...
    if (clear_width > 0 && !rlflag) {
        int startCol = col;
        while (col < clear_width && ScreenLines[off_to] == ' ' && ScreenAttrs[off_to] == 0) {
            ++off_to;
            ++col;
        }
        if (col < clear_width) {
            if (gui.in_use && (col > startCol || !redraw_this)) {
                ...
                {
                    int prev_cells = 1;
                    screen_char (off_to - prev_cells, row, col + coloff - prev_cells);
                }
            }
    ...
    if (clear_width > 0) {
        if (col + coloff < Columns) {
            int c;
            c = fillchar_vsep (&hl);
            if (ScreenLines[off_to] != (schar_T) c || ScreenAttrs[off_to] != hl) {
...
static int char_needs_redraw (int off_from, int off_to, int cols) {
    if (cols > 0 && ((ScreenLines[off_from] != ScreenLines[off_to] || ScreenAttrs[off_from] != ScreenAttrs[off_to])))
        return TRUE;
    return FALSE;
}
...
static void redraw_win_toolbar (win_T *wp) {
    ...
    for (menu = wp->w_winbar->children; menu != NULL; menu = menu->next)
        ++item_count;
    wp->w_winbar_items = (winbar_item_T *) alloc_clear ((unsigned ) sizeof (winbar_item_T) * (item_count + 1));
    for (menu = wp->w_winbar->children; menu != NULL && col < wp->w_width; menu = menu->next) {
    ...
    screen_line (wp -> w_winrow, wp -> w_wincol, (int) wp -> w_width, (int) wp -> w_width, FALSE);
}
...
static void fold_line (win_T *wp, long  fold_count, foldinfo_T *foldinfo, linenr_T lnum, int row) {
    ...
    if (fdc > 0) {
        ...
        if (wp->w_p_rl) {
            int i;
            copy_text_attr (off + wp -> w_width - fdc - col, buf, fdc, HL_ATTR (HLF_FC));
            for (i = 0; i < fdc; ++i)
    ...
    if (signcolumn_on (wp)) {
        ...
        if (len > 0) {
            if (len > 2)
                len = 2;
            if (wp->w_p_rl)
                copy_text_attr (off +wp->w_width - len - col, (char_u *) "  ", len, HL_ATTR (HLF_FL));
            else
        ...
        if (lnum >= top->lnum && lnume <= bot->lnum && (VIsual_mode != 'v' || ((lnum > top->lnum || (lnum == top->lnum && top->col == 0)) && (lnume < bot->lnum || (lnume == bot->lnum && (bot->col - (*p_sel == 'e')) >= (colnr_T) STRLEN (ml_get_buf (wp->w_buffer, lnume, FALSE))))))) {
            if (VIsual_mode == Ctrl_V) {
                if (wp->w_old_cursor_fcol + txtcol < (colnr_T) wp->w_width) {
                    ...
                    RL_MEMSET (wp -> w_old_cursor_fcol + txtcol, HL_ATTR (HLF_V), len - (int) wp -> w_old_cursor_fcol);
                }
            }
            else {
    ...
    if (wp->w_p_cc_cols) {
        ...
        while (j > -1) {
            ...
            if (txtcol >= 0 && txtcol < wp->w_width)
                ScreenAttrs[off + txtcol] = hl_combine_attr (ScreenAttrs[off + txtcol], HL_ATTR (HLF_MC));
            txtcol = old_txtcol;
            j = wp->w_p_cc_cols[++i];
        }
    }
    if (wp->w_p_cuc) {
        ...
        if (wp->w_p_wrap)
            ...
            txtcol -= wp->w_leftcol;
        if (txtcol >= 0 && txtcol < wp->w_width)
            ScreenAttrs[off + txtcol] = hl_combine_attr (ScreenAttrs[off + txtcol], HL_ATTR (HLF_CUC));
    }
    screen_line (row + W_WINROW (wp), wp -> w_wincol, (int) wp -> w_width, (int) wp -> w_width, FALSE);
