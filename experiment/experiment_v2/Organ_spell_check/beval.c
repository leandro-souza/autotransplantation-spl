...
void general_beval_cb (BalloonEval *beval, int state) {
    ...
    if (get_beval_info (beval, TRUE, &wp, &lnum, &text, &col) == OK) {
        bexpr = (*wp->w_buffer->b_p_bexpr == NUL) ? p_bexpr : wp->w_buffer->b_p_bexpr;
        if (*bexpr != NUL) {
            ...
            save_curbuf = curbuf;
            curbuf = wp->w_buffer;
            use_sandbox = was_set_insecurely ((char_u *) "balloonexpr", *curbuf->b_p_bexpr == NUL ? 0 : OPT_LOCAL);
            curbuf = save_curbuf;
            if (use_sandbox)
                ++sandbox;
            ++textlock;
...
int can_use_beval (void) {
    return (0 || (gui.in_use && p_beval) || (!gui.in_use && p_bevalterm)) && msg_scrolled == 0;
}
        ...
        if (!mouse_comp_pos (wp, &row, &col, &lnum)) {
            ...
            if (col <= win_linetabsize (wp, lbuf, (colnr_T) MAXCOL)) {
                if (getword) {
                    ...
                    if (VIsual_active && wp->w_buffer == curwin->w_buffer && (lnum == spos->lnum ? col >= (int) spos->col : lnum > spos->lnum) && (lnum == epos->lnum ? col <= (int) epos->col : lnum < epos->lnum)) {
                        ...
                        ++emsg_off;
                        len = find_ident_at_pos (wp, lnum, (colnr_T) col, &lbuf, FIND_IDENT +FIND_STRING + FIND_EVAL);
                        --emsg_off;
                        if (len == 0)
                            return FAIL;
